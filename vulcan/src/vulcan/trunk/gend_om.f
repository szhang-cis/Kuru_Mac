...
C
      PARAMETER(
     .   MNITV=1,      ! non-standard initial conditions - 0: no; 1: yes
     .   MPRE1=1,      ! pressure
     .   MPRE2=6,      ! comp. of stress (4 & 6 for NDIME=2 & 3)
     .   MPRE3=2,      ! comp. of int. var.
     .   MGALE=1,      ! Discontinuous Galerkin formulation
     .   MPRE4=6,      ! stress
     .   MPRE5=36 )    ! constitutive tensor
C
      PARAMETER(
     ...
     .   MSKEW=6 )           ! skew system (0:no; n:yes)
C
      PARAMETER(
     .   MACTI=0,        ! 0: no active elements; 1: active elements
     .   MOCOI=0 )       ! 0: coincident contact mesh; 1: non-coin. c.m.
C
      PARAMETER(
     .   MLDSF=0 )       ! 0: no def.-dep. load; 1: def.-dep. load
C
      PARAMETER(
     .   MANIS=1,        ! 0: isotropic models; 1: anisotropic models
     .   MANIV=3,        ! number of anisotropic vectors
     .   MANIC=4 )       ! number of components of anisotropic vectors
C
      PARAMETER(
     .   MDISR=1+2*MDYNA,                           ! dimension of DISPR
     .   MDISO=1+4*MDYNA,                           ! dimension of DISTO
     .   MPREA=MPRE1+(MPRE2+MPRE3)*MNITV+           ! dimension of PREAS
     .         (MPRE4+MPRE5)*MGALE )
C
...
C
     ...
     .   MFPCHM=0,   ! 2*number of phase-changes (see setdatt.f)
...
C
      PARAMETER(
     .   MMICRM=0,       ! 0:no microstructural coupling; 1:micr. coupl.
...
C
     ...
     .   MSUBF=50,             ! see setdat
     .   MPREL=22,             ! see setdat
c    .   MHIST=36,             ! 3D symm. const. tensor; see conset
c    .   MHIST=51,             ! 3D unsymm. const. tensor; see conset
     .   MHIST=52,             ! 3D unsymm. const. tensor; see conset
...
C**** 2) SOLVER TO BE USED:
C
C        MSOL1=1 & MSOL2=0 & MSOL3=0 & MSOL4=0: SKYLINE SOLVER
C        MSOL1=0 & MSOL2=1 & MSOL3=0 & MSOL4=0: FRONTAL SOLVER
C        MSOL1=0 & MSOL2=0 & MSOL3=1 & MSOL4=0: PCG SOLVER
C        MSOL1=0 & MSOL2=0 & MSOL3=0 & MSOL4=1: GMRES SOLVER
C
C        (change if necessary; see input data)
C
C***********************************************************************
...
C        gmres: always unsymmetric case
C        Therefore, for these solvers MUNS1 and MUNS2 are not used
C
C        (change if necessary; see input data)
C
C***********************************************************************
...
C
     ...
     .   MEQNS2=MDEF2*500000 )                ! chosen variable
C
      PARAMETER(
...
C
     ...
     .   MLAST2=MDEF2*48000000 )              ! chosen variable
c    .   MLAST2=MDEF2*60000000 )              ! chosen variable (GARUFA)
C
      PARAMETER(
...
C
     ...
     .   MFRON2=MDEF2*3100 )                  ! chosen variable
C
      PARAMETER(
...
C
     ...
     .   MBUFA2=MDEF2*1 )                    ! chosen variable
C
      PARAMETER(
...
C
      PARAMETER(
     .   MMKRYL1=MDEF1*(MEQNS/100+1),         ! default
     .   MMKRYL2=MDEF2*70 )                   ! chosen variable
C
      PARAMETER(
c    .   MMKRYL=max(MMKRYL1,MMKRYL2) )        ! sg
     .   MMKRYL=(MMKRYL1+MMKRYL2) )           ! PC & linux
C
      PARAMETER(
     .   MWORKG=MEQNS*(MMKRYL+1)+(MMKRYL*(MMKRYL+1))/2+
     .          4*MMKRYL+2 )
C
      PARAMETER(
     .   MCERO=0 )
