...
from Kuru.TimeIntegrators import ExplicitGrowthRemodelingIntegrator
...
class FEMSolver(object):
    ...
    def __checkdata__(self, material, boundary_condition, formulation, mesh, function_spaces, 
        ...
        if mesh is None:
            ...
            raise ValueError("mesh has to be an instance of Kuru.Mesh")
        ...
        if solver is None:
            solver = LinearSolver(linear_solver="direct", linear_solver_type="umfpack", 
                    geometric_discretisation=mesh.element_type)
        ...
        if "Explicit" in material.mtype:
            if self.analysis_subtype == "implicit":
                raise ValueError("Incorrect material model ({}) used for implicit analysis".\
                        format(material.mtype))
        ...
        # AT THE MOMENT ALL ELASTICITY TENSORS SEEMINGLY HAVE THE SAME SIGNATURE SO THIS IS O.K.
        try:
            F = np.eye(material.ndim,material.ndim)[None,:,:]
            material.GetElasticityTensor(KinematicMeasures(F,self.analysis_nature))
        except TypeError:
    ...
    def Solve(self, formulation=None, mesh=None,
        ...
        NodalForces, Residual = np.zeros((mesh.points.shape[0]*formulation.nvar,1),\
                dtype=np.float64),np.zeros((mesh.points.shape[0]*formulation.nvar,1),dtype=np.float64)
        ...
        if formulation.fields == "mechanics" and self.analysis_nature != "nonlinear":
            if self.analysis_type == "static":
                ...
                TotalDisp = self.IncrementalLinearElasticitySolver(function_spaces, formulation,
                        mesh, material, boundary_condition, solver, TotalDisp, Eulerx, NeumannForces)
        ...
        if self.analysis_type == "static":
            K, TractionForces, _ = Assemble(self, function_spaces, formulation, mesh, material,
            ...
            if self.analysis_subtype != "explicit":
                ...
                K, TractionForces, M = Assemble(self, fspace, formulation, mesh, material, 
                        boundary_condition, Eulerx)
            else:
    ...
    def NewtonRaphson(self, function_spaces, formulation, solver,
        ...
        while self.norm_residual > Tolerance or Iter==0:
            ...
            # SAVE THE NORM
            self.NRConvergence['Increment_'+str(Increment)] = np.append(self.NRConvergence['Increment_'+\
                str(Increment)],self.norm_residual)
    ...
    def LogSave(self, formulation, TotalDisp, Increment):
        ...
        if self.save_incremental_solution:
            ...
            if filename is not None:
                ...
                savemat(filename+"_"+str(Increment),{'solution':TotalDisp[:,:,Increment]},do_compression=True)
            else:
