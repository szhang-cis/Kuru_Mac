...
!loading history
  do inc=0,tstep   !number of steps
    ...
    stress: do !stress cycle
      ...
      call walltube(pressure,lambda1,stress_circ1,stress_long1)
      call walltube(pressure,lambda2,stress_circ2,stress_long2)
      !calculate stress from anisotropic model
      call stresses(time,pressure,lambda1,stress_theta1,stress_z1)
      call stresses(time,pressure,lambda2,stress_theta2,stress_z2)
      !jacobian matrix
      derivative=(stress_theta2-stress_circ2-stress_theta1+stress_circ1)/(lambda2-lambda1)
      !looking for new roots
      lambda0 = lambda1 - (stress_theta1-stress_circ1)/derivative
      call walltube(pressure,lambda0,stress_circ0,stress_long0)
      call stresses(time,pressure,lambda0,stress_theta0,stress_z0)
      !checking convergence
      if (abs((lambda1-lambda0)/lambda0)<tol) exit stress
      !updating roots
      lambda2 = lambda1
      lambda1 = lambda0
...
!writing
    ...
    write(10,100) inc,lambda_r,lambda0,pressure/two,stress_theta0,stress_z0
  end do !number of steps
100 format(i3,2f8.5,3f12.3)
  close(10)
  call system ('gnuplot plotting.plt')
end program tube
!****************subroutine stresses*********************************************
subroutine stresses(time,pressure,lambda_theta,stress_theta,stress_z)
  use parameters
  implicit none
  !exteral variables
...
!stress in circumferential direction
  stress_theta = stress_theta_e + stress_theta_c + stress_theta_m - pressure/two
...
!stress in longitudinal direction
  stress_z = stress_z_e + stress_z_c + stress_z_m - pressure/two
!************************************************************
end subroutine stresses
!**********subroutine walltube********************************************
subroutine walltube(pressure,lambda_theta,stress_circ,stress_long)
  use parameters
  implicit none
  real(kind=8),intent(in) :: pressure,lambda_theta
  real(kind=8),intent(out) :: stress_circ,stress_long
  real(kind=8) :: ri,t
