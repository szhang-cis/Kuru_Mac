      SUBROUTINE PROP05T(ITAPET,PROPST)
C***********************************************************************
C
...
C
      IF(NMEMO3.EQ.1) THEN
       NHISTY=7+1               ! see index of variables in micr05t.f
       NHISTX=NHISTT
...
C
      PROPST(1)=0.0D0           ! isotrt=0 => isotropic material
      PROPST(2)=0.0D0           ! ifill=0 => standard material
C
...
C
     ...
     . CALL RUNENDT('ERROR: WRONG NUMBER OF PHASE_CHANGES')
      ITERMEX=ITERMEF
      IF(ITERMEX.GT.1) ITERMEX=1
      NLINEA=(NFPCH-ITERMEX)/2              ! micro checked in pointes.f
      IF(NLINET.GT.NLINEA)
     . CALL RUNENDT('ERROR: WRONG NUMBER OF PHASE_CHANGES')
      IF(ITERME.GE.0) THEN
...
C
      ...
      NPOI1T=NPOI2T
      PROPST(NPOI2T)=0.0D0
      IF(WORDST(2).EQ.'LINEA') PROPST(NPOI2T)=1.0D0
...
c          CALL RUNMENT('WARNING: AD-PC/ISOLIF=1 TERM TREATED AS NI-PC')
         ...
         ENDIF
        ENDIF
c       IF(PARAMT(3).LT.0.0D0)     ! to allow exothermic reactions
