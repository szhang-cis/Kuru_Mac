...
def GrowthRemodelingResidual_eval(time,Delta_t,material,Stress_H,Y,elem,gcounter,constituent):
    """
    Return the value of the residual function on the point and its derivative
    """
    if constituent is 'ela':
        func=GrowthRemodeling_Residual(time=time,Delta_t=Delta_t,material=material,Stress_H=Stress_H,
            Y_pos=Y,elem=elem,gcounter=gcounter,fcount=0)
#        df = -1.0 - Delta_t/(101.0*365.25)
#        err = 0.0
        df,err = DGR_Ridders(time=time,Delta_t=Delta_t,material=material,Stress_H=Stress_H,
            Y_pos=Y,elem=elem,gcounter=gcounter,xcount=0,fcount=0)
    else:
        selec = np.zeros(2,dtype=np.int64)
        if constituent is 'smc':
            selec[0]=1
            selec[1]=6
        elif constituent is 'co1':
            selec[0]=2
            selec[1]=7
        elif constituent is 'co2':
            selec[0]=3
            selec[1]=8
        elif constituent is 'co3':
            selec[0]=4
            selec[1]=9
        elif constituent is 'co4':
            selec[0]=5
            selec[1]=10
        func = np.zeros(2,dtype=np.float64)
        df = np.zeros((2,2),dtype=np.float64)
        ...
        for j in range(2):
            func[j]=GrowthRemodeling_Residual(time=time,Delta_t=Delta_t,material=material,Stress_H=Stress_H,
                Y_pos=Y,elem=elem,gcounter=gcounter,fcount=selec[j])
            for i in range(2):
                df[j,i],err[j,i] = DGR_Ridders(time=time,Delta_t=Delta_t,material=material,Stress_H=Stress_H,
                    Y_pos=Y,elem=elem,gcounter=gcounter,
                    xcount=selec[i],fcount=selec[j])
    return func,df,err

def GrowthRemodeling_Newton(time,Delta_t,material,Stress_H,Y_,elem,gcounter,constituent):
    """
    Using the Newton-Raphson method, find the root of a function known to lie close to x. 
    The root rtnewt will be refined until its accuracy is known within +/- xacc. funcd
    is a user-supplied subroutine that returns both the function value and the first derivative
    ...
    if constituent is 'ela':
        # Set to maximum number of iterations.
        ITER=0
        Residual=GrowthRemodeling_Residual(time=time,Delta_t=Delta_t,material=material,Stress_H=Stress_H,
            Y_pos=Y_,elem=elem,gcounter=gcounter,fcount=0)
        norm_res = Residual
        if np.isclose(norm_res,0.0):
           norm_res = 1.0e-14
        norm_residual = np.absolute(Residual/norm_res)
        while norm_residual>1.0e-5 or ITER==0:
            Residual,Derivative,err = GrowthRemodelingResidual_eval(time=time,Delta_t=Delta_t,material=material,
                Stress_H=Stress_H,Y=Y_,elem=elem,gcounter=gcounter,constituent='ela')
            if np.isclose(Derivative,0.0):
                print('The Derivative for Newton-Raphson is zero')
                return True
            dx = Residual/Derivative
            material.GrowthRemodeling[0][elem][gcounter] -= dx
            # Convergence.
            ITER += 1
            norm_residual = np.absolute(Residual/norm_res)
            x = material.GrowthRemodeling[0][elem][gcounter]
            norm_solution = np.absolute(dx/x)
            if norm_solution<1.0e-5:
                break
            if np.isnan(norm_residual) or norm_residual>1.0e6 or ITER>20:
                print('newton_raphson diverge, elem= '+str(elem)+' gcounter= '+str(gcounter))
        ...
        for j in range(2):
            Residual[j]=GrowthRemodeling_Residual(time=time,Delta_t=Delta_t,material=material,Stress_H=Stress_H,
                Y_pos=Y_,elem=elem,gcounter=gcounter,fcount=selec[j])
        norm_res = np.linalg.norm(Residual)
        if np.isclose(norm_res,0.0):
            norm_res = 1.0e-14
        norm_residual = np.absolute(np.linalg.norm(Residual)/norm_res)
        while norm_residual>1.0e-5 or ITER==0:
            ...
            Derivative_inv = np.linalg.inv(Derivative)
            dx = np.dot(Derivative_inv,Residual)
            material.GrowthRemodeling[selec[0]][elem][gcounter] -= dx[0]
            material.GrowthRemodeling[selec[1]][elem][gcounter] -= dx[1]
            # Convergence.
            ITER += 1
            norm_residual = np.absolute(np.linalg.norm(Residual)/norm_res)
            x = np.zeros(2,dtype=np.float64)
            x[0] = material.GrowthRemodeling[selec[0]][elem][gcounter]
            x[1] = material.GrowthRemodeling[selec[1]][elem][gcounter]
            norm_solution = np.absolute(np.linalg.norm(dx)/np.linalg.norm(x))
            if norm_solution<1.0e-5:
                break
            if np.isnan(norm_residual) or norm_residual>1.0e6 or ITER>20:
    ...
    for elem in range(1):
        for gcounter in range(1):
            ...
            Newton_failed = GrowthRemodeling_Newton(time=time,Delta_t=Delta_t,material=material,
                Stress_H=Stress_H,Y_=Y_,elem=elem,gcounter=gcounter,constituent='ela')
            if Newton_failed:
                print('Newton method failed at constituent: '+'ela')
                print('Newton method failed at Gauss point: '+str(gcounter))
                break
            if Iter is 0:
                continue
            for key in ['smc','co1','co2','co3','co4']:
                Newton_failed = GrowthRemodeling_Newton(time=time,Delta_t=Delta_t,material=material,
                    Stress_H=Stress_H,Y_=Y_,elem=elem,gcounter=gcounter,constituent=key)
                if Newton_failed:
                    print('Newton method failed at constituent: '+key)
                    break
            if Newton_failed:
                print('Newton method failed at Gauss point: '+str(gcounter))
                break
