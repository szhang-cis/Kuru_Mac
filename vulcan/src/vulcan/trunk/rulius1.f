      SUBROUTINE RULIUS1(NGAUS,NDIME,POSGP,WEIGP)
C***********************************************************************
C
...
C
      IF(NDIME.EQ.1) THEN
       IF(NGAUS.EQ.2) THEN
c       call runendt('rulius1: not implemented')
        POSGP(1,1)=-1.0D0
	POSGP(1,2)= 1.0D0
C
        WEIGP(1)=1.0D0
...
C
      IF(NDIME.EQ.2) THEN
C
       IF(NGAUS.EQ.1) THEN ! idem rultri.f (to avoid smoothing problems)
c       call runendt('rulius1: not implemented')
...
C**** QUADRILATERALS
C
       IF(NGAUS.EQ.4) THEN          ! Lobatto type
c       call runendt('rulius1: not implemented')
        POSGP(1,1)=-1.D+00          ! compatible with rulgau.f
        POSGP(2,1)=-1.D+00
        POSGP(1,2)=-1.D+00
        POSGP(2,2)= 1.D+00
        POSGP(1,3)= 1.D+00
...
C
	...
	WEIGP(4)  = 1.D+00
       ...
       ENDIF
C
       IF(NGAUS.EQ.9) THEN          ! Lobatto type
c       call runendt('rulius1: not implemented')
        POSGP(1,1)=-1.D+00          ! compatible with rulgau.f
        POSGP(2,1)=-1.D+00
        POSGP(1,2)=-1.D+00
...
C
       IF(NGAUS.EQ.4) THEN          ! Lobatto type
c       call runendt('rulius1: not implemented')
        POSGP(1,1)= 0.0             ! compatible with rultri.f ??
        POSGP(2,1)= 0.0
        POSGP(3,1)= 0.0
        POSGP(1,2)= 1.0
        POSGP(2,2)= 0.0
...
C
       IF(NGAUS.EQ.6) THEN
        ...
        POSGP(2,4)= 1.D+00/6.D+00
        POSGP(3,4)= A
        POSGP(1,5)= 2.D+00/3.D+00
        POSGP(2,5)= 1.D+00/6.D+00
        POSGP(3,5)= A
        POSGP(1,6)= 1.D+00/6.D+00
        POSGP(2,6)= 2.D+00/3.D+00
        POSGP(3,6)= A
C
        WEIGP(1)  = 1.D+00/6.D+00
        WEIGP(2)  = 1.D+00/6.D+00
...
C**** BRICKS
C
       IF(NGAUS.EQ.8) THEN          ! Lobatto type
c       call runendt('rulius1: not implemented')
        POSGP(1,1)=-1.D+00          ! compatible with rulgau.f
        POSGP(2,1)=-1.D+00
        POSGP(3,1)=-1.D+00
...
C
       ...
       ENDIF
C
       IF(NGAUS.EQ.27) THEN         ! Lobatto type
c       call runendt('rulius1: not implemented')
        POSGP(1,1)= -1.D+00         ! compatible with rulgau.f
        POSGP(2,1)= -1.D+00
        POSGP(3,1)= -1.D+00
