      SUBROUTINE PRIDIRN(NTYPE,STRSG,STRSP)
C***********************************************************************
C
C**** THIS ROUTINE COMPUTES THE PRINCIPAL DIRECTION OF THE 
C     STRESS TENSOR ( 2D & 3D ) 
C
C     Note: this routine is identical to pridir.f with the exception
C           that pridirn.f does not order the eigenvalues
C
C***********************************************************************
...
C
c     DATA TWOPI,ZM/6.283185307179586,1.0E-15/
...
c     IF(PSTR1.GE.PSTR2) THEN
       STRSP(1)=PSTR1
       STRSP(2)=PSTR2
       STRSP(3)=ANGLE
c     ELSE
c      ANGLE=ANGLE+TWOPI/4.0D+00
c      STRSP(1)=PSTR2
c      STRSP(2)=PSTR1
c      STRSP(3)=ANGLE
c     ENDIF
