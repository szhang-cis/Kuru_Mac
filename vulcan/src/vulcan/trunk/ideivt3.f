      SUBROUTINE IDEIVT3(PROPS)
C***********************************************************************
C
C**** THIS ROUTINE ORDER THE PROPERTIES FOR THE ISOTROPIC,
C     VISCOPLASTIC & TEMPERATURE-DEPENDENT MODEL FOR DUAL PHASE STEEL
C
C***********************************************************************
C
C     NYOUNF
...
C
C     NYOUNA
C     VYOUNA(1)=Young's Modulus of martensite   at VYOUNA(2)=temperature
C
C     NPOISF
C     VPOISF(1)=Poisson's Ratio of ferrite      at VPOISF(2)=temperature
C
C     NPOISA
C     VPOISA(1)=Poisson's Ratio of martensite   at VPOISA(2)=temperature
C
C     NALPHF
...
C
      DIMENSION PROPS(*)
C
C**** FERRITE & MARTENSITE FRACTIONS
C
      I1F=61
      FRACT(1)=PROPS(I1F)
      I1F=I1F+1
...
C
      DO IPART=1,NPART
       IA=I1F-1+2*IPART
       IB=IA+1
       VPART(IPART,1)=PROPS(IA)
       VPART(IPART,2)=PROPS(IB)
      ENDDO
C
C**** YOUNG MODULUS
C
