      SUBROUTINE FRUC05T(CARTDT,DVOLUT,EHISTT,ELCODT,ELDIST,EPMTXT,
...
C
     ...
     .   TGAUST=TALFAT*TGAUST+(1.0D0-TALFAT)*(TGAUST-DTEMPT*DTIMET)
       ENDIF
C
C**** UPDATE THE DENSITY & TEMPERATURE DERIVATIVE OF f_pc
C
       ILAHET=2
       ISINRT=2
C
     ...
     .               DSOURT,COUTDT, BASMI,TGAUIT,PSEUDO)
       ELSE
        ...
        IF(IMICR.EQ.1) THEN
         ...
         COEFLT=EHISTT(8+IX,IGAUST) ! L*(df_pc/dT) computed in micr05t.f
c        COEFLT=EHISTT(9+IX,IGAUST) ! L*(df_pc/dT) computed in micr05t.f
        ELSE
         IF(MULPHT.EQ.1) THEN
          CALL CAPCOFT(BASMM ,BASCC ,PROPST,TGAUST,TGAUSX,
...
C
       IF(IMICR.EQ.0) THEN
        ...
        FACT1T=0.0D0
        VELA1T=DABS(DTEMPT)
        IF(VELA1T.GT.VELTOT) FACT1T=COEFLT
        IF(LINPC.EQ.1) FACT1T=COEFLT
       ELSE
        FACT1T=COEFLT
       ENDIF
C
