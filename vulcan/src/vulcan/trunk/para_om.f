...
C
      PARAMETER(
     . MPREV1=1*MMEMOMX,                      ! STRA0(NSTR1,NGAUS)
     . MPREV2=MPREV1+MSTR1*MGAUS*MMEMOM,      ! STRS0(NSTR1,NGAUS)
     . MPREV3=MPREV2+MSTR1*MGAUS*MMEMOMX,     ! TEMPC(4)
...
C
     ...
     . mprin8=mprin7+(MTOTV*MCHAL+4)/8*       ! PRESC(NTOTV,2)
     .        (1+MPOREB/2)*(1+MACTI),
     ...
     . mprin14=mprin13+MTOTV*(1+MMEMO9M),     ! DISPR(NTOTV,NDISR)
     . mprin15=mprin14+MTOTV*(1+MDISR),       ! DISTO(NTOTV,NDISO)
     . mprin16=mprin15+MTOTV*MDISO,           ! HEADS(NPOIN,4)
     . mprin17=mprin16+MPOIN*4*MPOREC,        ! REFOR(NTOTV,2)
     . mprin18=mprin17+MTOTV*2,               ! TLOAD(NTOTV,2)
     . mprin19=mprin18+MTOTV*2,               ! LPNTN(NPOIN)
     . mprin20=mprin19+                       ! ELDAT(NDATA)
     .         (MPOIN*MCHAL+4)/8*MRENU,
     ...
     . mprin27=mprin26+                       ! COFRI(NSKEW,NDIME,NDIME)
     .         (MPOIN*MCHAL+4)/8*MSKE1,
     . mprin28=mprin27+MSKEW*MDIME*MDIME,     ! PWORK(NPOIN,2)
     . mprin29=mprin28+                       ! PREAS(NPREA,NPOIN)
     .         MPOIN*(1+MITERCTM)*MTER2M,
     . mprin30=mprin29+MPREA*MPOIN*MTER1M,    ! TGAPS(NPOIN)
     ...
     . mprin34=mprin33+                       ! NOPRF(NNODE,NELEM)
     .         (MELEM*MCHAL+4)/8*MACTI,
     . mprin35=mprin34+                       ! PRESF(NNODE,NDIME,NELEM)
     .         (MNODE*MELEM*MCHAL+4)/8*MLDSF,
     . mprin36=mprin35+                 ! PREHF(NNODE,NDIME,NELEM,NFUNC)
     .         MNODE*MDIME*MELEM*MLDSF )
      PARAMETER(
     . mprin37=mprin36+                       ! VANIS(NANIV,NANIC,NELEM)
     .         MNODE*MDIME*MELEM*MFUNC*MLDSF,
     . MPRIN  =mprin37+MANIV*MANIC*MELEM*MANIS )
C
C**** RSSETP
C
      PARAMETER(
...
C
     ...
     . IFOT42=IFOT41+MDIME*MGAUS,             ! DVOL2(NGAUS)
     . IFOT43=IFOT42+MGAUS*MOCOA,             ! ELCO2(NDIME,NNODE)
     . IFOT44=IFOT43+MDIME*MNODE*MOCOA,       ! GPCO2(NDIME,NGAUS)
     . IFOT45=IFOT44+MDIME*MGAUS*MOCOA,       ! SHAP2(NNODE,NGAUS)
     . IFOT46=IFOT45+MNODE*MGAUS*MOCOA,       ! ELEDG(NDIME,NNODE)
     . IFOT47=IFOT46+MDIME*MNODE*MOCOA,       ! VTANL(NDIME,NDIM1,NNODG)
     . IFOT48=IFOT47+                         ! STDGA(NPRE4,NNODE)
     .        IFOT47+MDIME*(MDIME-1)*MNODG*MOCOA,
     . IFOT49=IFOT48+MPRE4*MNODE*MGALE,       ! CTDGA(NPRE5,NNODE)
     . LFOT  =IFOT49+MPRE5*MNODE*MGALE )
C
      PARAMETER(
     . IFOR1=1,                               ! ELELM(NEVAB)
...
C
     ...
     . ILOS7=ILOS6+(MNODE*MCHAL+4)/8,         ! POSGP(NDIME,NGAUS)
...
C
     ...
     . ISET21=ISET20+MDIME*MGAUS,             ! COOSM(NDIME,NGAUS)
     . ISET22=ISET21+MDIME*MGAUS*MOCOI,       ! COOSS(NDIME,NGAUS)
     . ISET23=ISET22+MDIME*MGAUS*MOCOI,       ! VNOSM(NDIME,NGAUS)
     . ISET24=ISET23+MDIME*MGAUS*MOCOI,       ! DVOL2(NGAUS)
     . ISET25=ISET24+MGAUS*MOCOI,             ! ELCO2(NDIME,NNODE)
     . ISET26=ISET25+MDIME*MNODE*MOCOI,       ! GPCO2(NDIME,NGAUS)
     . ISET27=ISET26+MDIME*MGAUS*MOCOI,       ! SHAP2(NNODE,NGAUS)
     . ISET28=ISET27+MNODE*MGAUS*MOCOI,       ! DISI2(NEVAB)
     . ISET29=ISET28+MEVAB*MOCOI,             ! DISP2(NEVAB)
     . ISET30=ISET29+MEVAB*MOCOI,             ! VTANL(NDIME,NDIM1,NNODG)
     . ISET31=ISET30+MDIME*(MDIME-1)*MNODG*MOCOI,         ! GAPMI(NGAUS)
     . ISET32=ISET31+MGAUS*MOCOI,             ! STREI(NPRE2,NNODE)
     . ISET33=ISET32+MNITV*MPRE2*MNODE,       ! VARII(NPRE3,NNODE)
     . LSET  =ISET33+MNITV*MPRE3*MNODE )
C
      PARAMETER(
     ...
     . ISTI34=ISTI33+MEVAB*MEVAB*MSKE1,       ! EMATX(NEVAB,NEVAB)
     . ISTI35=ISTI34+MEVAB*MEVAB,             ! EMATX(NEVAB,NEVAB)
     . ISTI36=ISTI35+MEVAB*MEVAB,             ! DVOL2(NGAUS)
     . ISTI37=ISTI36+MGAUS*MOCOA,             ! ELCO2(NDIME,NNODE)
     . ISTI38=ISTI37+MDIME*MNODE*MOCOA,       ! GPCO2(NDIME,NGAUS)
     . ISTI39=ISTI38+MDIME*MGAUS*MOCOA,       ! SHAP2(NNODE,NGAUS)
     . ISTI40=ISTI39+MNODE*MGAUS*MOCOA,       ! DISI2(NEVAB)
     . ISTI41=ISTI40+MEVAB*MOCOA,             ! VTANL(NDIME,NDIM1,NNODG)
     . ISTI42=ISTI41+                         ! CTDGA(NPRE5,NNODE)
     .        MDIME*(MDIME-1)*MNODG*MOCOA,
     . LSTI  =ISTI42+MPRE5*MNODE*MGALE )
C
      PARAMETER(
...
C
      PARAMETER(
     . ISOL1G=1+LSTI*MMEMO7M,                 ! GSTDI(NEQNS)
     . ISOL2G=ISOL1G+MEQNS,                   ! GSTLO(NLAST*IUNSY)
     . ISOL3G=ISOL2G+MLAST*MUNS2*MCERO,       ! GSTUP(NLAST)
     . ISOL4G=ISOL3G+MLAST*MCERO,             ! CSTIF(NEVAC,NEVAC)
     . ISOL5G=ISOL4G+MEVAC*MEVAC,             ! ELOAD(NEQNS)
     . ISOL6G=ISOL5G+MEQNS*MCERO,             ! LNUEQ(NTOTV)
     . ISOL7G=ISOL6G+(MTOTV*MCHAL+4)/8,       ! LPONT(NTOTV)
     . ISOL8G=ISOL7G+(MTOTV*MCHAL+4)/8,       ! DISIM(NEVAC)
     . ISOL9G=ISOL8G+MEVAC*MCERO,             ! FOREL(NEVAC)
     . ISOL10G=ISOL9G+MEVAC*MCERO,            ! ALOAD(NEQNS*IFITE)
     . ISOL11G=ISOL10G+MEQNS*MFITE*MCERO,     ! DELTA(NEQNS*IFITE)
     . ISOL12G=ISOL11G+MEQNS*MFITE*MCERO,     ! DISIC(NEQNS*IFITE)
     . ISOL13G=ISOL12G+MEQNS*MFITE*MCERO,     ! LOCAL(NEVAC*IFITE)
     . ISOL14G=ISOL13G+(MEVAC*MFITE*MCHAL+4)/8*MCERO,
     . ISOL15G=ISOL14G,                       ! BGMRE(NEQNS)
     . ISOL16G=ISOL15G+MEQNS,                 ! XGMRE(NEQNS)
     . ISOL17G=ISOL16G+MEQNS,                 ! WGMRE(NEQNS)
     . LSOLG  =ISOL17G+MWORKG )
C
      PARAMETER(
     . LSOL4=MSOL4*LSOLG )
C
      PARAMETER(
c    . LSOL=max(LSOL1,LSOL2,LSOL3,LSOL4) )    ! SG
     . LSOL=(LSOL1+LSOL2+LSOL3+LSOL4) )       ! PC & linux
C
