...
template<typename U>
class _NearlyIncompressibleNeoHookean_ : public _MaterialBase_<U> {
public:
    U mu;
    U kappa;

    FASTOR_INLINE _NearlyIncompressibleNeoHookean_() = default;

    FASTOR_INLINE
    _NearlyIncompressibleNeoHookean_(U mu, U kappa) {
        this->mu = mu;
        this->kappa = kappa;
    }
...
template<> template<>
void _NearlyIncompressibleNeoHookean_<Real>::KineticMeasures<Real>(Real *Snp, Real* Hnp,
