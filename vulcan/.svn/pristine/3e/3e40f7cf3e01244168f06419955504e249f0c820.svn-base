      SUBROUTINE LISTEN8(SUBNA,NPRIN,ITAPE)
C***********************************************************************
C
...
C
      CHARACTER SUBNA*6, CARD*81 ! Must be 80+1
C
      LOGICAL*1 NEWLINE
C
      SAVE KPRIN
...
C
       ...
       LASTP=0
C
       READ(ITAPE,'(A80)',END=2) CARD ! Read a card from input file.
c      LENG=LNBLNK(CARD)              ! Calculate the length.
       DO I=80,1,-1                   ! Calculate the length.
        IF(CARD(I:I).NE.' ') THEN
         LENG=I
         GO TO 9999
        ENDIF
...
C
       IF(WORDS(1).EQ.'ECHO') THEN ! Deal with echo
        IF(WORDS(2).EQ.'OFF') THEN
         KPRIN=0
         WRITE(LURES,1) 'LISTEN','ECHO OFF'
        ELSE
         KPRIN=1
         WRITE(LURES,1) 'LISTEN','ECHO ON'
        ENDIF
        NNWOR=0                  ! Forget all
        NNPAR=0
        DO I=1,MAXWP
       ...
       ELSE                      ! Print card
        IF((KPRIN.EQ.1).AND.(IFIRSP.LE.LASTP)) THEN
         IF(NEWLINE) THEN
          ...
          CARD(LASTP-1:LASTP)=' \'
         END IF
         IF(NPRIN.EQ.0) WRITE(LURES,1) SUBNA,CARD(IFIRSP:LASTP)
        END IF
       ENDIF
C
...
C**** ERRORS:
C
    2 IF(SUBNA.NE.'LOADPS'.AND.ITAPE.NE.LULOA) THEN
       CALL RUNEND('LISTEN:END OF FILE DETECTED        ')
      ELSE
       IENDOF=1
       RETURN
      ENDIF
    3 CALL RUNEND('LISTEN: TOO MANY PARAM IN COMMAND  ')
    4 CALL RUNEND('LISTEN: TOO MANY WORDS IN COMMAND  ')
C
      END
