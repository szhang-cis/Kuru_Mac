      SUBROUTINE VIMODL(SGTOT,EBASE,DMATX,PROPS,
...
C     Notes:
C
C     h_Cp: plastic hardening coefficient (CCOEF)
...
C
     ...
     .          VECTGL(6),      VECTFL(6)
      ...
      DIMENSION TRMTX(NDIME,*), DSTRA(*), DSTRAL(6)
...
C
C**** COMPUTATION OF INVARIANTS & EFFECTIVE STRESS (PREYS)
C
...
C
     ...
     .             CCERO,DAMAG)
      ENDIF         ! index.eq.1
C
C**** COMPUTATION OF THE "ABETA" MULTIPLICATOR & "DLAMD"
C
      IF(INDEX.EQ.1) THEN
       CALL VIFLOW(VECTF,VECTFL,ABETA,DMATP,HARDS,NSTRS,NTYPE,
     ...
     .             SPTOT,TRMTX,SGTOTL,DSTRA,DSTRAL)
      ENDIF         ! index.eq.1
C
