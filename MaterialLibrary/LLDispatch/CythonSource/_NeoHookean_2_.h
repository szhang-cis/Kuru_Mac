...
public:
    ...
    FASTOR_INLINE
    _NeoHookean_2_(U mu, U kappa, U pressure) {
        this->mu = mu;
        this->kappa = kappa;
    ...
    FASTOR_INLINE
    void SetParameters(U mu, U kappa, U pressure){
        this->mu = mu;
        this->kappa = kappa;
        this->pressure = pressure;
    }
    ...
    _KineticMeasures_(const T *Fnp, int near_incomp) {
    ...
    template<typename T>
    void KineticMeasures(T *Snp, T* Hnp, int ndim, int ngauss, const T *Fnp, int near_incomp);

};

template<> template<>
void _NeoHookean_2_<Real>::KineticMeasures<Real>(Real *Snp, Real* Hnp,
    int ndim, int ngauss, const Real *Fnp, int near_incomp) {
    ...
    if (ndim==3) {
        ...
        for (int g=0; g<ngauss; ++g) {
            std::tie(stress,hessian) =_KineticMeasures_<Real,3>(Fnp+9*g, near_incomp);
        ...
        for (int g=0; g<ngauss; ++g) {
            std::tie(stress,hessian) =_KineticMeasures_<Real,2>(Fnp+4*g, near_incomp);
