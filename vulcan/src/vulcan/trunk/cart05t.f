      SUBROUTINE CART05T(ELCODT,PROPST,ELDIST,SHAPET,CARTDT,
     .                   POSGPT,WEIGPT,GPCODT,SHAPDT,DERIVT,XJACMT,
     .                   CARDDT,VELCMT,WHAPEL,HACHET,WARTDL,
     .                   WERIVT,CENTRO,ADVEMT,TEINIT,FPCHLT,
     .                   EHISTT)
C***********************************************************************
C
C**** THIS ROUTINE COMPUTES GALERKIN (IGALET=0) OR PETROV-GALERKIN
C     (IGALET=1) TYPE WEIGHT FUNCTIONS & THEIR CARTESIAN DERIVATIVES
C     ( ELEMENT NO. 5 )
C
C     Notes: to compute some perturbation functions (according to IPERT)
C            or the cartesian derivatives of the perturbation
C            functions, it is necessary to calculate the isoparametric
C            jacobian (SHAPDT & CARDDT are auxiliars variables).
C
C***********************************************************************
...
C
     ...
     .          WHAPEL(NNODLT,*),
     .          HACHET(NNODLT),          WARTDL(NDIMET,NNODLT,*),
      ...
      IF(IGALET.EQ.0) THEN
       DO IGAUST=1,NGAULT
        DO INODLT=1,NNODLT
         ...
         DO IDIMET=1,NDIMET 
          WARTDL(IDIMET,INODLT,IGAUST)=CARTDT(IDIMET,INODLT,IGAUST)
         ENDDO
        ENDDO
       ENDDO
       RETURN
...
C
      CALL WACOBST(WARTDL(1,1,IGAUST),WERIVT(1,1,IGAUST),
     .             NDIMET,NNODLT,XJACMT)
C
...
C
     ...
     .                       WHAPEL(INODLT,IGAUST)
       DO IDIMET=1,NDIMET
        WARTDL(IDIMET,INODLT,IGAUST)=CARTDT(IDIMET,INODLT,IGAUST)+
     .                               WARTDL(IDIMET,INODLT,IGAUST)
       ENDDO
      ENDDO
C
  100 CONTINUE           ! igaust=1,ngault
C
      RETURN
      END
