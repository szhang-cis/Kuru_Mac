      SUBROUTINE ELM033(LNODS,PROEL,PROPS,INFRI,COFRI,NOPRF,PRESF,WORK1,
...
C     ELEMENT NO. 33 :
C
C     2D/3D ELEMENT CONTACT ELEMENT
C
C     FUNCTIONS:
C
...
C
      IF(NPOIC.EQ.0) THEN
       CALL MASS33(WORK1(ISTIF(11)),                       ! DVOLU
     .             PROPS,LNODS,
...
C
     ...
     .             WORK1(IFORC(47)),                       ! VTANL
     .             WORK1(IFORC(38)),WORK1(IFORC(15)),      ! DISI2,DISPL
     .             WORK1(IFORC(48)),WORK1(IFORC(49)))      ! STDGL,CTDGL
      ELSE
       IF(IAUGM.EQ.0.OR.IAUGM.EQ.1) THEN
