...
public:
    ...
    _KineticMeasures_(const T *Fnp, const T *Nnp, int nfibre, const T *SVnp)
    {
	...
	T coeff0 = std::pow(J_e,-2./3.);
        Tensor<T,ndim,ndim> stress = (mu*coeff0*(b_e-1./3.*trb*I) + kappa*J_e*(J_e-1.)*I)*SVnp[14]/J;
	...
	auto bI_ijkl = einsum<Index<i,j>,Index<k,l>>(b_e,I);
        ...
        Tensor<T,ndim,ndim,ndim,ndim> elasticity = (2.*mu*coeff0*(1./9.*trb*II_ijkl-
			...
			kappa*J_e*((2.*J_e-1.)*II_ijkl-(J_e-1.)*(II_ikjl+II_iljk)))*SVnp[14]/J;
    ...
    void KineticMeasures(T *Snp, T* Hnp, int ndim, int ngauss, const T *Fnp, int nfibre,
		    const T *Nnp, int nstatv, const T *SVnp);

};

template<> template<>
void _ArterialWallMixture_<Real>::KineticMeasures<Real>(Real *Snp, Real* Hnp, int ndim, int ngauss, 
		const Real *Fnp, int nfibre, const Real *Nnp, int nstatv, const Real *SVnp)
{
    ...
    if (ndim==3) {
        ...
        for (int g=0; g<ngauss; ++g) {
            std::tie(stress,hessian) =_KineticMeasures_<Real,3>(Fnp+9*g, Nnp, nfibre, SVnp+nstatv*g);
        ...
        for (int g=0; g<ngauss; ++g) {
            std::tie(stress,hessian) =_KineticMeasures_<Real,2>(Fnp+4*g, Nnp, nfibre, SVnp+nstatv*g);
