...
def ComputeStress(material,elem,gcounter,stress,Stress_i,Softness):
    ...
    for key in ['co1','co2','co3','co4','smc']:
        ...
        if key is 'smc':
            den0_tot = 1050.0
            s_act = 54.0e3*material.mix_density['smc'][elem][gcounter]
            stretch_m = 1.4
            stretch_a = 1.0
            stretch_0 = 0.8
...
def ElastinDepositionStretchThin(material,R_gauss,nelem,NoGauss):
    """ 
    ...
    print('==> Looking for Radial Deposition Stretch by Thin-Walled')
    for elem in range(nelem):
        ...
        for gcounter in range(NoGauss):
            ...
            while norm_residual>1.0e-6 or Iter==0:
                ...
                Residual[0] = pressure*(0.5-(R_gauss[elem][gcounter]-R)/H) + \
                2.*kappa*stretch_r*stretch_t*stretch_z*(stretch_r*stretch_t*stretch_z-1.)/J + \
                mu3D*(stretch_r*stretch_t*stretch_z)**(-2./3.)*(stretch_r**2-(stretch_r**2+stretch_t**2+stretch_z**2)/3.)/J
                Residual[1] = 2.*kappa*stretch_r*stretch_t*stretch_z*(stretch_r*stretch_t*stretch_z-1.)/J + \
                mu3D*(stretch_r*stretch_t*stretch_z)**(-2./3.)*(stretch_t**2-(stretch_r**2+stretch_t**2+stretch_z**2)/3.)/J + \
                (s_act/den0_tot)*(1.-((1.4-1.0)/(1.4-0.8))**2.)/J + \
                c1m*(stretch_m**2-1.)*np.exp(c2m*(stretch_m**2-1.)**2)*stretch_m**2/J + \
                (c1c1+c1c2/2.+c1c3/2.)*(stretch_c**2-1.)*np.exp(c2c*(stretch_c**2-1.)**2)*stretch_c**2/J - \
                pressure*R/H
                # compute the derivative
                Jacobian[0,0] = 2.*kappa*(2.*stretch_r*(stretch_t*stretch_z)**2-stretch_t*stretch_z)/J - \
                (2./3.)*mu3D*stretch_r**(-5./3.)*(stretch_t*stretch_z)**(-2./3.)*(stretch_r**2-(stretch_r**2+stretch_t**2+stretch_z**2)/3.)/J + \
                4.0*mu3D*(stretch_r*stretch_t*stretch_z)**(-2./3.)*stretch_r/(3.0*J)
                Jacobian[0,1] = 2.*kappa*(2.*stretch_t*(stretch_r*stretch_z)**2-stretch_r*stretch_z)/J - \
                (2./3.)*mu3D*stretch_t**(-5./3.)*(stretch_r*stretch_z)**(-2./3.)*(stretch_r**2-(stretch_r**2+stretch_t**2+stretch_z**2)/3.)/J - \
                2.0*mu3D*(stretch_r*stretch_t*stretch_z)**(-2./3.)*stretch_t/(3.0*J)
                Jacobian[1,0] = 2.*kappa*(2.*stretch_r*(stretch_t*stretch_z)**2-stretch_t*stretch_z)/J - \
                (2./3.)*mu3D*stretch_r**(-5./3.)*(stretch_t*stretch_z)**(-2./3.)*(stretch_t**2-(stretch_r**2+stretch_t**2+stretch_z**2)/3.)/J - \
                2.0*mu3D*(stretch_r*stretch_t*stretch_z)**(-2./3.)*stretch_r/(3.0*J)
                Jacobian[1,1] = 2.*kappa*(2.*stretch_t*(stretch_r*stretch_z)**2-stretch_r*stretch_z)/J - \
                (2./3.)*mu3D*stretch_t**(-5./3.)*(stretch_r*stretch_z)**(-2./3.)*(stretch_t**2-(stretch_r**2+stretch_t**2+stretch_z**2)/3.)/J + \
                4.0*mu3D*(stretch_r*stretch_t*stretch_z)**(-2./3.)*stretch_t/(3.0*J)
...
def ElastinDepositionStretchThick(material,R_gauss,nelem,NoGauss):
    """ 
        This routine aim to get the radial deposition stretch from the boundary conditions 
        in radial direction, as stress=-p in inner wall and stress=0 in outer wall. The 
        relation for the stresses along the thickness come from thin-walled tube. So radial
    ...
    print('==> Looking for Radial Deposition Stretch by Thick-Walled')
    for elem in range(nelem):
        if NewtonRaphson_failed:
            print('ElastinDepositionStretch stop at elem'+str(elem))
            break
        ...
        for gcounter in range(NoGauss):
            ...
            Residual = np.zeros(2,dtype=np.float64)
            Jacobian = np.eye(2,2,dtype=np.float64)
            Residual[0] = pressure*Ri**2*(1.-Ro**2/R_gauss[elem][gcounter]**2)/(Ro**2-Ri**2)
            Residual[1] = -pressure*Ri**2*(1.+Ro**2/R_gauss[elem][gcounter]**2)/(Ro**2-Ri**2)
            NormForces = np.linalg.norm(Residual)
            norm_residual = np.absolute(np.linalg.norm(Residual)/NormForces)
            while norm_residual>1.0e-6 or Iter==0:
                # compute the residual
                ...
                pressure*Ri**2*(1.+Ro**2/R_gauss[elem][gcounter]**2)/(Ro**2-Ri**2)
                # compute the derivative
                ...
                Jacobian[1,0] = 2.*kappa*(2.*stretch_r*(stretch_t*stretch_z)**2-stretch_t*stretch_z) - \
                ...
                Jacobian[1,1] = 2.*kappa*(2.*stretch_t*(stretch_r*stretch_z)**2-stretch_r*stretch_z) - \
...
def ElastinDepositionStretch(material,nelem,NoGauss):
    """ 
        This routine aim to get the radial deposition stretch from the boundary conditions 
        in radial direction, as stress=-p in inner wall and stress=0 in outer wall. The 
        relation for the stresses along the thickness come from thin-walled tube. So radial
...
#=================  HOMEOSTATIC SOLUTION  =======================
    ...
    ElastinDepositionStretchThin(material=material, R_gauss=R_gauss, nelem=mesh.nelem, NoGauss=NoGauss)
