...
from Kuru.FiniteElements.AssemblyShell import Assemble #, AssembleExplicit
...
class ShellSolver(object):
    ...
    def __checkdata__(self, material, boundary_condition,
        ...
        if mesh.points.shape[1] == 3 and (mesh.element_type == "tri" or mesh.element_type == "quad") and formulation.fields != "shell_mechanics":
        ...
        if formulation.fields == "shell_mechanics":
        ...
        if boundary_condition.dirichlet_flags is not None:
            ...
            if formulation.fields == "shell_mechanics":
                if boundary_condition.dirichlet_flags.shape[1] != (ndim+2):
                    raise ValueError("Essential boundary conditions are not imposed correctly")
    ...
    def Solve(self, formulation=None, mesh=None,
        ...
        Eulerr = np.zeros(mesh.points.shape[0]*(formulation.nvar-formulation.ndim))
        ...
        if formulation.fields == "shell_mechanics" and self.analysis_nature != "nonlinear":
            if self.analysis_type == "static":
        ...
        if self.analysis_type == "static":
            K, TractionForces, _ = Assemble(self, function_spaces, formulation, mesh, material, boundary_condition, Eulerx, Eulerr)
        else:
