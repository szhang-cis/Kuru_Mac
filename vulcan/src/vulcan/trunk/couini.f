...
C
C**** CHECKS RESTART OR PREVIOUS ANALYSIS
C
      IF(IREST.EQ.1.OR.INITI.EQ.2) RETURN
C
C**** GLOBAL CONVERGENCE FOR OUTPUT
C
      NCKGLO=0
C
C**** COMPUTES INITIAL MICROSCOPIC PHASE-CHANGE FUNCTION
C
      IF(IMICR.EQ.1) THEN
       NOUTPA=NOUTPT(20)                           ! stores NOUTPT(20)
       NOUTPT(20)=1
       IF(IPRCOT.EQ.0)
     ...
     .               WORK1T)
       NOUTPT(20)=NOUTPA                           ! restores NOUTPT(20)
      ENDIF
C
C**** START INITIAL TIME INTERVAL FOR THE MECHANICAL PROBLEM
C
...
C
C**** TRANSFER INITIAL THERMAL VARIABLES TO MECHANICAL PROBLEM
C
...
C
      ENDDO   ! END OF TIME STEPS LOOP FOR INITIAL COND. FOR MECH. PROB.
C
C**** INITIAL COUPLING MECHANICAL TERM
C
      IF(ITERME.GT.0) THEN
       DO IPOIN=1,NPOIN
        PWORK(IPOIN,1)=0.0
