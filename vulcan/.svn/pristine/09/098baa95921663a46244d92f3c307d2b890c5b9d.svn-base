      SUBROUTINE MASS32Z(DVOLU,PROPS,SHAPE,WSTIF,EHIST,ELDIS,TENOD,
...
C
      ...
      ENDIF
C
C**** COMPUTES NORMAL PRESSURE IN THE GLOBAL SYSTEM (CURRENT)
C
      PRESN=0.0
      DO INODL=1,NNOBO
      ...
      END DO
C
C**** COMPUTES NORMAL PRESSURE IN THE GLOBAL SYSTEM (LAST CONVERGED)
C
      PRESA=0.0
      DO INODL=1,NNOBO
       ELDIA=ELDIS(1,INODL+2*NNOBO)-DISPL(1,INODL+2*NNOBO)
       PRESA=PRESA+SHAPE(INODL,IGAUS)*ELDIA
      END DO
C
...
C
      ...
      IF(TGAUS.LT.TEMPL) THEN
       IF(PRESN.GT.PREST) THEN
        DO III20=1,20
         IF(IITER.GT.(III20*IITEF)) RIGI3=RIGI3*TRUPL
        ENDDO
        IF(PRESA.GT.PREST) RIGI3=RIGI3*TRUPM
       ENDIF
...
C
     ...
     .            KSYMA,AUXS1)
C
C**** COMPUTES SECOND CONTACT CONSTITUTIVE TENSOR (UNSYMMETRIC)
C
C     Note: normal component only (not tangential contributions)
C
      DO IDIME=1,NDIME
       PROYI=VNORL(IDIME,IGAUS)
       DMATX(IDIME,1)=RIGI3*PROYI
      ENDDO
C
C**** COMPUTES CONTACT MATRIX (DISPLACEMENT-FORCE) (UNSYMMETRIC)
C
     ...
     .            KSYMA,AUXS4)
C
C**** COMPUTES THIRD CONTACT CONSTITUTIVE TENSOR (SYMMETRIC)
C
      DO IDIME=1,NDIME
       DO JDIME=1,NDIME
        DMATX(IDIME,JDIME)=0.0D+00
...
C
     ...
     .            AUXS1,AUXS2,AUXS3,AUXS4,IMODEL)
C
  100 CONTINUE
C
      RETURN
