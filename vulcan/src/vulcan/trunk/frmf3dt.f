      SUBROUTINE FRMF3DT(PROPST,VELCMT,ELELMT,DVOLUT,SHAPET,EHISTT,
...
C
       ...
       if(nr.eq.1) ng=1
       if(nr.eq.2) ng=7
       if(nr.gt.2) call runendt('error in frmf3dt with nnodlt=4')
c
       do ig=1,ng
        if(nr.eq.1) then
         x1=0.0
...
C
         CALL PHCH12T(PROPST,VELCMT,ELELMT,SHAPDT,DVOLDT,EHISTT,ELDIST,
     .                DISIMT,ELELTT,
...
C
     ...
     .             FR123*FSS13*FTT23 + FR123*FSS23*FTT13
          ...
          CALL PHCH08T(PROPST,VELCMT,ELELMT,SHAPDT,DVOLDT,EHISTT,ELDIST,
...
C
      IF(NNODLT.EQ.10) THEN
       CALL RUNENDT('FRMF3DT:ERROR DETECTED WITH NDIME=3')
       RETURN
      ENDIF
C
