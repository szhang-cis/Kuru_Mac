...
c      imicr=1          ! these parameters shoul be read
c      imico=1
...
c 1000 CALL LISTENC('COUINP',IPRINC,ITAPEC)
C
C**** IDENTIFY COMMAND
C
c      DO ICOMM=1,MCOMM
...
c      ITERMP=0
c      ITERMD=0
c      IF(ITERME.GT.0) THEN                     ! bidirectional coupling
c       ITERMG=1                                ! defaults
...
C
c      RETURN 
c 2000 CALL RUNEND('COUINP: ERROR IN COUPLED DATA BLOCK')
c 2001 CALL RUNEND('COUINP: NO STOP CARD IN COUPLED DATA BLOCK')
C
