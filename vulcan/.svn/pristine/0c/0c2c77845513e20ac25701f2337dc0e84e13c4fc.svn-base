      SUBROUTINE STMC3DT(CARTDT,DVOLUT,EHISTT,ELDIST,EPMTXT,GPCODT,
...
C***********************************************************************
C
C**** THIS ROUTINE COMPUTES THE ADVECTIVE PHASE CHANGE MATRIX (LATENT
C     HEAT CONTRIBUTION INTO THE JACOBIAN MATRIX FOR ELEMENT NO. 5 )
C     FOR NDIME=3
C
C***********************************************************************
...
C
       ...
       if(nr.eq.1) ng=1
       if(nr.eq.2) ng=7
       if(nr.gt.2) call runendt('error in mamf3dt with nnodlt=4')
c
       do ig=1,ng
        if(nr.eq.1) then
         x1=0.0
...
C
         CALL LAHC11T(CARTDT,DVOLUT,EHISTT,ELDIST,EPMTXT,GPCODT,
     .                PROPST,SHAPET,STRSGT,ESTIFT,HSTIFT,BMATXT,
     .                DMATXT,SIGMAT,XJACMT,
     .                SHAPDT,DVOLDT,
     .                POSGPT,WEIGPT,
...
C
          CALL LAHC08T(CARTDT,DVOLUT,EHISTT,ELDIST,EPMTXT,GPCODT,
     .                 PROPST,SHAPET,STRSGT,ESTIFT,HSTIFT,BMATXT,
     .                 DMATXT,SIGMAT,XJACMT,
     .                 SHAPDT,DVOLDT,
     .                 POSGPT,WEIGPT,
...
C
      IF(NNODLT.EQ.10) THEN
       CALL RUNENDT('MAMF3DT:ERROR DETECTED WITH NDIME=3')
       RETURN
      ENDIF
C
