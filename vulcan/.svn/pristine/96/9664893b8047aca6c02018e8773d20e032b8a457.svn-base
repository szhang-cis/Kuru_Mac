      SUBROUTINE FRODEST(LOCEL,LNODS,NACVA,NBUFA,NDEST,NDOFN,NELEM,
...
C**** OPEN FILE .frhk
C
      OPEN(UNIT=LUFRHT,FILE=CDT,STATUS='UNKNOWN',FORM='UNFORMATTED')
ctm
...
C
      REWIND LUFRHT
      WRITE(LUFRHT) NBUFA,NFRON,NSTIF
C
      DO 1 IFRON=1,NFRON
    1 NACVA(IFRON)=0
C
...
C
      IF(KFRON.GT.NFRON)
     . CALL RUNENDT('ERROR IN FRODEST: KFRON            ')
C
C**** STORE IN TAPE ELEMENT DESTINATION
C
      WRITE(LUFRHT) LOCEL,NDEST,KFRON
      WRITE(LUFRHT) (NACVA(IACVA), IACVA=1,KFRON)
C
