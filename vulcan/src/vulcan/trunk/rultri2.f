      SUBROUTINE RULTRI2(NDIME,NGAUS,POSGP,WEIGP)
C***********************************************************************
C
...
C
      IF(NDIME.EQ.2) THEN
       IF(NGAUS.EQ.1) THEN
c       call runendt('rultri2: not implemented')
...
C
       ...
       ENDIF
       IF(NGAUS.EQ.3) THEN
        POSGP(1,1)= 1.D+00/2.D+00   !  this was the original in rultri.f
        POSGP(2,1)= 0.D+00
        POSGP(1,2)= 1.D+00/2.D+00
        POSGP(2,2)= 1.D+00/2.D+00
        POSGP(1,3)= 0.D+00
        POSGP(2,3)= 1.D+00/2.D+00
C
