 SUBROUTINE pcte08 (flag,iload)
 !
 !  Write mesh information for TECPLOT for 3_D BEAM elements
 !
 USE data_db
 IMPLICIT NONE
 ...
 CALL tecgetgv(flag,gv,beame_nodes,beame_nps,ngv)  !get global variables
 IF( flag) CALL tecgenvarshls(6)   !Generate Variables Share List and Passive Variables list
 nlgv = t_ev(2,6)                  !number of Local Gauss Variables
 ...
 DO iset=1,beame_sets            !for each element set
   ...
   WRITE(11,"('ZONE T=',a,' Nodes=',i6,' Elements=',i6,' ZONETYPE=',a15,    &
              ' DATAPACKING=BLOCK  SOLUTIONTIME=',e12.4  )") &   !,' STRANDID=',i4
               sname(1:l+2),beame_nps, nelem, etype, ttime       !, iload
   ...
   IF( flag )THEN !SOLUTION
     ...
     IF( one )THEN                  !only print for first set
       DO i=1,ngv    !print global variables
         WRITE(11,"(10e15.6)")(gv(i,n),n=1,beame_nps)
         WRITE(11,"()")            !leave a line between data blocks
       END DO
       DO i=1,beame_nvarn !print smoothed variables
         WRITE(11,"(10e15.6)")(beame_vargs(i,n),n=1,beame_nps)
         WRITE(11,"()")            !leave a line between data blocks
       END DO
   ...
   ELSE           !GRID
     ...
     DO iel = 1,nelem            !for each element in the set
       IF( e%nnode == 2 )THEN
         WRITE(11,"(4i8)") beame_nodes(e%lnods(:,iel),1)  !print local numeration
       ELSE
         WRITE(11,"(4i8)") beame_nodes(e%lnods(1:2,iel),1)  !print local numeration
         WRITE(11,"(4i8)") beame_nodes(e%lnods(2:3,iel),1)  !print local numeration
       END IF
     END DO
   END IF
   one = .FALSE.
